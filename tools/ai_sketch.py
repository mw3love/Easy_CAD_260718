...
def build_from_image(image_path: str, out_path: str, *, api_key: str = "", note: str = "",
                      ...
                      dark: bool = True, verbose: bool = True) -> dict:
    ...
    if verbose:
        print(f"[P1 개괄] {ov.model_used} ({ov.elapsed:.1f}s) "
    ...
    if total_items <= tile_threshold:
        ...
        for i, box in enumerate(tiles):
            ...
            try:
                ...
                if verbose:
                    print(f"[P2 타일 {i}] 실패: {e}")
                continue
            tdata = gw.parse_json(res.content)
            if verbose:
                print(f"[P2 타일 {i}/{len(tiles)}] {res.model_used} ({res.elapsed:.1f}s) "
    ...
    if verbose:
        print(f"[P3 완료] shapes={len(shapes)} edges={len(edges)} unknown={len(unknown)} "
