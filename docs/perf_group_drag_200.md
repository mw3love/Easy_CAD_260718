...
## ⚠ 후속 시도(2026-08-13, 같은 날) — `_sync_geom_snapshot` 재확인 + `paint()` 캐싱 시도·되돌림

이어서 실제 프로파일로 확인한 결과 `_sync_geom_snapshot`은 전체 비용의 4%뿐이라(위
"남은 것"의 예상과 달리) **다음 병목이 아님을 확정** — 씬≠선택 별도 재현 문서는 필요
없다. 실제 남은 비용의 대부분(약 70%)은 `_paint_selection_highlight`가 다중선택 도형마다
매 paint() 호출 시 `_highlight_band()`(스트로크+`simplified()`+`subtracted()`, 값비싼
`QPainterPath` 불리언 연산)를 처음부터 재계산하는 것 — `_PathItem`(자유곡선)은 이미
`self._sel_outline` 캐시를 갖고 있지만 `_HandleResizeMixin`의 기본 구현(사각형·원·심볼이
씀)에는 캐시가 없었다.

같은 "값 비교" 패턴(`content_rect`·펜폭·cuts/ports를 키로)으로 캐시를 구현해 스모크는
잘 통과했으나, **전체 pytest 스위트(543종)를 여러 번 돌리자 무작위 지점에서 프로세스가
크래시**(exit 127, `faulthandler` 활성화해도 트레이스백 없음 — 진짜 네이티브 크래시로
추정)했다. 대조 실험으로 원인이 이 변경임을 확인:
- 이 변경 있음: 전체 스위트 2/2 크래시(매번 다른 위치 — 66%대, 79%대 등 비결정적)
- 이 변경 없음(직전 커밋): 전체 스위트 3/3 클린 통과
- 파일 하나만 격리 실행하면 재현 안 됨(누적 리소스에 의존하는 것으로 보임) — 이
  프로젝트 CLAUDE.md에 이미 기록된 "Windows 세그폴트(위젯 수 증가가 기존 잠재
  재현조건을 앞당김)"와 같은 계열일 가능성.

**되돌림 — 커밋 안 됨.** 근본 원인 미상(네이티브 크래시라 Python 트레이스백으로 특정
불가). 다음에 이 최적화를 다시 시도한다면:
- `QPainterPath` 인스턴스를 `QGraphicsItem` 인스턴스 속성으로 장기 보관하는 것 자체가
  의심 지점(이미 `_PathItem._sel_outline`이 같은 패턴을 쓰고 있어 그 자체가 원인은
  아닐 가능성도 있음 — 차이점을 더 좁혀야 함).
- 재현이 비결정적(누적 리소스 의존)이라 파일 단위 격리 테스트로는 못 잡는다 — 반드시
  전체 스위트를 여러 번 돌려 확인할 것.
- `faulthandler`로도 안 잡히므로 Windows 이벤트 뷰어(Application 로그) 또는
  `ProcDump`(Sysinternals) 같은 외부 크래시 덤프 도구가 필요할 수 있음.
