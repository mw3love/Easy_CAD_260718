...
### Phase 3 — host_* UI 레이어 코드리뷰 — **완료 (2026-08-25/26)**
...
- [x] `host_fileio.py`(1,187줄) — **완료(2026-08-26, 재실행 성공)**. 7건 발견, 2건
      반영(커밋 `a8698b1`): `_do_open_ecad`가 도형 로드 성공 후 레이어 읽기만 실패해도
      전체를 "열기 실패"로 되돌려 `_doc_path` 없는 미완성 문서가 남던 버그(레이어
      읽기를 별도 격리해 기본 레이어 폴백), SVG 가져오기 전체 실패 시 경고+상태바
      중복 메시지. 1건 재분류(DXF 안내 플래그 타이밍 — docstring이 "이후는 조용히
      진행"이라 명시한 확정 설계로 확인, 버그 아님). 4건 보류(Mermaid `build_elbow()`
      bare except — host_mindmap과 동일 패턴 일관 보류, 팔레트 그룹드래그 grab 유실
      — 발동조건 극히 좁음, 죽은 import ~49개 — 규칙 8 보고만, 중복 상수 7개 파일
      — Phase 7 이관).
...
**완료 기준 충족**: 4개 파일(`host_dialogs`·`host_ui`·`host_widgets`·`host_fileio`) +
9개 묶음, 총 findings 18건(4+0+2+7+5) 전부 분류·처리(반영 7 / 재분류·기각 1 / 보류 10)
+ 1차 시도 미검증분 6건 별도 폐기(세션 한도로 취합 실패). 전체 스위트 1010종 통과.
...
### Phase 6 — Not-tested 백로그 트리아지 + 실사용 검증 시나리오표 — **완료 (2026-08-26)**

커밋 본문 `Not-tested:` 실측 **353건**(계획 당시 추산 342건과 근접, git 로그가 그 뒤로도
계속 쌓여 소폭 증가). `git log --all --grep="^Not-tested:" -E` 로 재현 가능(집계 방법은
아래 §6-1).

**§6-1 방법론** — 353건 전부를 Read로 정독 후 키워드+문맥 판정으로 4분류. 순수 키워드
매칭(마우스/손맛/실기기/데스크톱 등)으로 1차 필터링한 188건은 즉시 버킷1 확정, 나머지
165건(13건은 "없음"→버킷3, 152건은 개별 판정)은 각 커밋의 이후 히스토리(같은 기능이
후속 커밋에서 실제로 재작업·재검증됐는지)를 CLAUDE.md 진행기록과 대조해 분류했다 —
353건 전부를 지금 코드로 재실행 검증한 것은 아니고(그럴 경우 각각이 별도 회귀조사가
필요해 현실적 범위를 벗어남), **커밋 히스토리상 그 우려가 해소된 근거가 있는지**를
판정 기준으로 삼았다(확인 필요 항목은 버킷1/4로 보수적으로 남김).

**4분류 집계**:

| 버킷 | 개수(근사) | 처리 |
|---|---|---|
| 1. 손맛/환경 전용(대리 불가) | 약 260건 | §6-3 시나리오표로 이관 |
| 2. 자동화 가능한데 안 한 것 | 2건(`_BadgeItem.apply_color` 외 1건은 이미 이전 Phase에서 회귀테스트로 닫힘) | `_BadgeItem.apply_color`는 이 Phase에서 즉시 반영(커밋 `ed26a6e`) |
| 3. 이미 무효(코드 변경·후속 검증 완료) | 약 75건 | 장부에서 제외(웹 프로토타입 트랙 전체(2026-08-06 중단, ~20건) + AI 게이트웨이 "실키 미검증" 항목 중 후속 세션에서 실키 검증 완료된 것(~15건) + 기능 자체가 이후 완전히 재구현/대체된 것(~25건) + "스코프 밖 확정"으로 이미 문서화된 것(~15건)) |
| 4. 진짜 미해결 | 3건 → 재조사 후 1건만 순수 미해결로 확정(아래 §6-2) | §6-2 참조 |

**§6-2 알려진 미해결 항목 재확인 — 재조사 결과**:

| 항목 | 재조사 결과 | 판정 |
|---|---|---|
| DXF 왕복 화살촉 개수(`head_start`) 보존 | 이 세션 Phase 4에서 재현·수정 완료(커밋 `11d05ca`) | ✅ **해결됨** |
| PDF 용지크기/방향 "안 먹힘" | `pdf_export._resolve_geometry`/`_PdfExportDialog._refresh` 코드 확인 — 표제란/용지틀이 씬에 있으면 그 프레임의 크기·방향이 항상 우선한다는 **의도된 설계**(2026-08-07 deep-interview 확정)이고, 다이얼로그가 이미 그 상태를 크기/방향 콤보 비활성화 + "표제란 용지 설정을 따름" 안내 라벨로 명확히 표시하고 있음을 코드로 확인 | ❌ **버그 아님** — 설계된 동작, UI 안내도 이미 존재. 사용자가 겪은 "안 먹힘"은 십중팔구 씬에 남아있는(의식 못 한) 표제란 때문일 가능성이 높음 |
| "내 심볼" 개별 썸네일이 옅게 보이는 문제 | 실제 창 렌더+픽셀 확대로 원인 규명(스크린샷 확인) — **렌더링 결함이 아니라 색상 대비 문제**다. `_render_symbol_thumbnail`이 등록된 도형의 실제 잉크색(예: 사용자가 검정 펜으로 그렸다면 `#111111`)을 그대로 쓰는데, 기본 팔레트 아이콘(`_shape_icon`)은 테마 적응 흰색을 쓴다 — 다크 캔버스에서 어두운 색으로 그린 심볼을 등록하면 그 어두운 색이 그대로 썸네일에 박혀 잘 안 보인다 | ⚠ **원인 규명 완료, 수정은 보류** — "썸네일을 흰색으로 강제"는 실제로 캔버스에 배치했을 때의 진짜 색(어두운 색)과 미리보기가 달라지는 트레이드오프가 있어(WYSIWYG 위반), 자동 색보정 여부는 디자인 판단이 필요해 사용자 확인 후 착수(아래 참조) |
| 마인드맵 빈 노드 취소 시 남는 고아 화살표 | `host_mindmap.py` 코드 확인 — `mm_create_child`/`mm_create_sibling`/`mm_create_in_direction` 전부 새 노드+화살표를 **즉시 커밋**(`push_undo_add_many`) 후 편집모드 진입, 편집을 빈 텍스트로 취소해도 되돌리는 코드가 없음을 확인 | ⚠ **여전히 미해결, 재현 조건 확인 완료** — 2026-08-24 도입 당시 "의도적 최소범위"로 남겨둔 결정 그대로. 수정하려면 "이 편집 세션이 방금 생성한 빈 노드의 첫 편집인지"를 추적하는 상태가 새로 필요해(단순 버그 수정이 아니라 작은 기능 추가) 사용자 확인 후 착수 |

**미해결 2건(심볼 썸네일 대비·마인드맵 고아 정리)은 설계 판단이 필요해 이 세션에서
바로 구현하지 않았다** — 둘 다 "언젠가 할 일" 목록(§8 계획서)에 남기고, 착수는 다음
세션 deep-interview로.

**§6-3 실사용 검증 시나리오표**: `docs/user_test_scenarios.md`(신규) — `python run.py`로
훑는 기능별 최소경로 체크리스트, 버킷1(손맛 전용) 항목들을 여기 흡수했다.

**완료 기준 충족**: 4분류 집계표 + 알려진 미해결 4항목 재조사 결과표(§6-2) +
`docs/user_test_scenarios.md`. 신규 pytest 1종(버킷2 닫음), 전체 스위트 1018종 통과.
다음: Phase 7(수정 반영 마무리·릴리스 준비·마감).
...
- **2026-08-26** — **Phase 3 완료**. host_dialogs.py(4건→2반영)·host_ui.py(0건)·
  host_widgets.py(2건→1반영)·host_fileio.py(7건→2반영·1재분류)·9파일 묶음(5건→2반영,
  1차 시도 6건은 세션한도로 취합실패해 미검증 폐기). 큰 발견: `_do_open_ecad`가 도형
  로드 성공 후 레이어 읽기만 실패해도 미완성 문서가 남던 버그, Mermaid 확대창 타이핑
  중 포커스 탈취, 팔레트 버튼 오른쪽클릭 드래그 오판, 고아 `@staticmethod` 3곳 정리.
  신규 pytest 9종, 전체 스위트 1010종 통과. 세션 한도로 여러 번 끊겼으나(백그라운드
...
- **2026-08-26 (후속 5)** — **Phase 6 완료**. 커밋 `Not-tested:` 353건 전부 Read로
  정독해 4분류(손맛/환경 전용 ~260·자동화 가능한데 안 함 2·이미 무효 ~75·진짜
  미해결 3→재조사 후 1). 알려진 미해결 4항목 재조사: ⓐ DXF 화살촉개수는 이 세션
  Phase 4에서 이미 해결 확인 ⓑ PDF 용지크기/방향 "안 먹힘"은 `_resolve_geometry`/
  `_PdfExportDialog._refresh` 코드 확인 결과 **버그 아님**(표제란이 있으면 그 설정을
  따르는 의도된 동작, UI가 이미 비활성화+안내라벨로 명시) ⓒ "내 심볼" 썸네일 흐림은
  실제 창 렌더+픽셀 확대로 재현해 원인 규명 — **렌더링 결함이 아니라 색상 대비
  문제**(등록 도형의 실제 잉크색을 그대로 써서 다크 캔버스에 어두운 색으로 그린
  심볼은 안 보임), 수정은 WYSIWYG 트레이드오프가 있어 디자인 판단 필요해 보류
  ⓓ 마인드맵 고아 화살표는 `host_mindmap.py` 코드 확인으로 재현조건 재확인(2026-08-24
  "의도적 최소범위" 결정 그대로 미해결) — 둘 다 다음 세션 deep-interview 후보로
  계획서에 남김. 버킷2(자동화 가능) 유일한 항목(`_BadgeItem.apply_color` 전용
  테스트 없음)은 즉시 반영(커밋 `ed26a6e`). 신규 문서 `docs/user_test_scenarios.md`
  (실사용 검증 시나리오표, 14개 기능 영역). 신규 pytest 1종, 전체 스위트 1018종
  통과. 다음: Phase 7(수정 반영 마무리·릴리스 준비·마감).
