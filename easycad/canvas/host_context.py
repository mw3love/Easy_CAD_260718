    ...
    def finalize_closed_trim(self, hosts: dict, undo_key=None):
        ...
        for host, before_cuts in hosts.items():
            ...
            if not results:
                ...
                ops.append(("remove", host))
                ops.append(("mut", host, "cuts", list(before_cuts), cuts))
                for port in ports:
            ...
            if host.has_label() and host._label is not None:
                ...
                if txt:
                    first.ensure_label().setPlainText(txt)
                    first._sync_label()
            self._scene.removeItem(host)
            for frag in new_items:
                self._scene.addItem(frag)
            ops.append(("remove", host))
            ops.append(("mut", host, "cuts", list(before_cuts), cuts))
            ops.extend(("create", frag) for frag in new_items)
            selected_after.extend(new_items)
            if ports:
                first._ports = ports
                for port in ports:
                    port.setParentItem(first)
                    port.setPos(first.mapFromScene(port_scene_pos[port]))
                    port._port_host = first
            for arr, idx, before in befores:
                self._rebind_arrow_to_group(arr, idx, new_items)
                arr.update()
                ops.append(("mut", arr, "geom", before, arr.capture_geom()))
        if not ops:
            return
        self._push_entry(ops, key=undo_key)
        self._scene.clearSelection()
        for it in selected_after:
            it.setSelected(True)
        self._refresh_properties()
