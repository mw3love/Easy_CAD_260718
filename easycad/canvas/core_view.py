...
class _AnnotatorView(QGraphicsView):
    ...
    def _try_commit_trim(self, kind: str, host, seg) -> None:
        ...
        [TRIM 파괴적 재설계 1~2단계, 2026-08-30] `_RectItem`/`_EllipseItem`은 더 이상
        비파괴 `_cuts`+`push_undo_cut`로 끝내지 않는다 — 드래그 중엔 지금까지와 똑같이
        `_cuts`에 누적만 해(미리보기 렌더 무변경) `_trim_closed_dirty`에 표시해두고, 실제
        확정(점목록으로 굽기/완전 소실 삭제)은 제스처가 끝나는 `_mouse_release_impl`의
        `finalize_closed_trim` 한 번으로 미룬다(같은 드래그에서 여러 변을 잘라도 병합
        로직 한 번으로 정확히 처리하기 위해 — 자세한 이유는 `_closed_shape_trim_fragments`
        참조). 그 외 닫힌 도형(심볼·닫힌다각형)은 아직 비파괴 그대로(다음 단계에서 순차
        전환)."""
        if kind == "closed":
            edge_i, t0, t1 = seg
            ...
            if key in self._trim_seen:
                return
            if isinstance(host, (_RectItem, _EllipseItem)):
