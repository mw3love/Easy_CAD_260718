...
- **웹 전환 실험 진행 중(2026-08-05 시작, 결론 미정)** — `web_prototype/`(이 리포 하위, PyQt
  본체와 별개 코드, `easycad/`는 안 건드림)에 포트/커넥터+지속연결 상호작용을 SVG+DOM으로
  재현해 "playwright 자동검증만으로 버그를 화면 없이 잡을 수 있는가" 가설을 검증 중. 1차:
  자동검증만으로 실버그 1건 발견+수정(A* 경로 그리드반올림 대각선 꺾임). 2차: 지속연결(도형
  이동→화살표 재라우팅) 1차 구현에서 버그 없이 통과. 추천 방향은 전면 재작성이 아니라
  하이브리드(DXF/PDF는 기존 Python 유지, 캔버스/인터랙션만 웹). 로컬로 보려면 `web_prototype/`
  에서 `python -m http.server 8791` 후 `localhost:8791/index.html`. **다른 PC에서 이어가려면**:
  아직 실제 손맛(hover/drag 느낌)은 검증 안 됨 — 브라우저로 직접 써보고 계속 여부 최종 판단.
  경위·근거는 `docs/history/2026-08.md` "웹 전환 실험" 항목, memory
  `project_web_pivot_experiment` 참조.
