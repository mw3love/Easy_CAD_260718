...
- **Phase 2** 진행:
  ...
  - **근접정렬 흡수(Stage4)** 완료(프록시+자체렌더 ✓, 실조건 대기): `[A]`백엣지 재진입·`[B]`하단
    수렴부의 작은 계단은 **연결 도형 중심축이 몇 px 어긋나면 직교 경로가 그 차이만큼 짧은 점프를
    반드시 넣는** 기하적 필연으로 진단(라우터 버그 아님 — 정렬 시 계단 0, 실측). 근본 해법:
    `_PolyArrowItem._absorb_near_alignment`(build_elbow 라우팅 직전) — 양끝 같은 방향 포트 + 교차축
    어긋남 ≤ `_ALIGN_TOL`(8px)이면 부착점을 공통 축(중점)으로 스냅해 계단을 직선으로 붕괴. 부착점
    (bind_pt 로컬)+`_pts` 끝점 함께 갱신(reroute가 안 되돌림), 멱등(스냅 후 어긋남≈0), 혼합 L자·
    의도적 큰 오프셋(>8px)은 미변경. 데이터/워크플로 정렬 대신 코어 흡수 채택(손그림·DXF 도형에도
    보편적용). 스모크 `test_sarrow_absorbs_near_alignment`. **잔여(open):** 라벨-선 겹침(`[C]`) —
    라벨 오프셋/배치 계열, 후속 진단→수정 예정.
