...
4. (계획서 §5 권장 흡수) — **직교 커넥터+자동라우팅·심볼 라이브러리 완료**. 남은 것:
   - ~~심볼/스텐실 라이브러리~~ — **완료**(6종 dock 팔레트 + 외곽선 스냅 + 중앙 라벨). 후속:
     진짜 드래그앤드롭, 심볼 종류 추가.
   - **포트/접속점** — 도형의 명시적 접속 지점. (미착수 — 현재는 화살표 지속연결만 있음)
5. **상단바 종합정리**(계획만, "나중에") — 네모·원 버튼을 왼쪽 팔레트로 이동해 "닫힌 도형은 다 왼쪽"
   으로 일관화(원은 곡선 기하 유지, 배치만 통일). 상단바는 그리기 도구만 남김. 상세: 메모리 `toolbar-cleanup-plan`.
