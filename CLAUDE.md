...
— 상세 로드맵 `docs/EasyCAD_계획.md` §Phase 6.
  ...
  - **M3 빠른 편집 UX** 완료(실조건검증 ✓ 2026-07-22, 커밋 `ca71d21`~`568065d`):
    ...
    **Phase 6 M3 완료.**
  - **M4 편집 정밀화 & 커넥터 고도화** 진행 — **M4-1/2/3 구현·프록시검증(스모크 158+자체렌더), 실조건
    대기**(커밋 `68b9c8f`·`04e11e2`·`bdb670b`, ⚠ 아직 run.py 미확인 — 검증 후 이 줄을 실조건검증 ✓로 갱신):
    ...
    remove+create+화살표 geom을 단일 undo 엔트리로 묶음(`_swap_shape`). **M4-4 직교 커넥터 고도화(sarrow
    무조건 직교+세그먼트 드래그)=남음** — 최대 위험(핵심 인터랙션 개편, M4-2 스냅과 커플링), 설계부터.
    ⚠ 화살표 범위는 방향 토글만 유지(#4 곡선↔직선 통합은 M4-4서 재검토).
