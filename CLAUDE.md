...
- **Phase 3** 진행: DXF 상호운용(`ezdxf`).
  ...
  - **실사용자 DXF 실조건검증 → 버그 6건 발견·수정 완료**(2026-07-29, 실조건검증 ✓ — 사용자가
    실제 방송국 결선도 DXF를 AutoCAD에서 저장해 `Ctrl+Shift+I`로 가져와 확인). 재현 파일은
    사용자 로컬(`C:\Users\minwoo\Desktop\D232323rawing1.dxf`, 리포 밖). 발견 경로: INSERT/BLOCK
    흡수를 실제 도면으로 첫 검증하다 원 계획(심볼 활용) 밖의 근본 문제 6개를 연쇄로 발견.
    - **ⓐ 외부 도면 자동 재스케일** — 이 도면은 INSUNITS=mm, 전체 88×36단위(㎜)로 우리 앱의
      고정폭 핸들·선택박스·기본 펜두께(전부 "도면이 수백~수천 단위"라는 암묵 전제) 대비 너무
      작아, 100% 줌에선 우표만하고 확대하면 그 고정폭들이 상대적으로 거대해 보였다. 손안의
      카드: PDF 출력이 애초에 절대단위 무관 "용지 fit" 방식이라 가져오기 시점 재스케일이 인쇄에
      영향 없음 — 확인함. `_compute_import_scale()`이 전체 bbox를 재 우리 앱 기준(`_TARGET_EXTENT`
      =1000)으로 균일 배율 계산, `_uf()`가 좌표에 일괄 적용. **우리 자신이 export한 파일(EC_*
      레이어 존재)은 절대 재스케일 안 함**(왕복 정밀도 보존, 스케일 1.0 유지) — 판정은 EC_*
      레이어 존재 여부. ⚠ 함정: 텍스트 bbox에 실제 렌더 폭까지 넣으면(60자 URL 주석 하나가
      도형 전체보다 넓어) 스케일이 왜곡됨 — 실측 확인 후 텍스트는 삽입점만 반영(의도적 여백
      허용). **ⓑ DXF 가져오기 후 자동 화면맞춤(`_zoom_fit()`)** — 외부 도면은 원점·스케일이
      우리와 무관해 가져온 직후 화면 밖이거나 안 보일 수 있어 열기 직후 항상 전체 맞춤.
      **ⓒ `SOLID` 엔티티 지원 추가** — 화살촉으로 흔한 채워진 삼각형(`SOLID`) 16개가 그동안
      지원 누락으로 조용히 사라지고 있었다. `_solid_item()`(정점 순서 1-2-4-3 보정 + 채움 처리)
      신설. **ⓓ 텍스트 앵커(halign/valign/attachment_point) 반영** — 그동안 삽입점을 무조건
      "좌상단"으로 읽어, DXF 스펙상 가운데·기준선 등 다른 정렬의 텍스트가 전체적으로 아래로
      치우쳐 보였다. TEXT의 halign/valign→align_point, MTEXT의 attachment_point(1~9)별 실제
      렌더 bbox 비율로 앵커 오프셋을 계산해 정확한 정렬점에 배치(회전축도 같은 오프셋으로 고정).
      **ⓔ ACI/ByLayer 색상 해석 + 화면흰색·인쇄검정** — 대부분의 외부 도면은 `true_color`가
      아니라 ACI(AutoCAD Color Index)나 ByLayer로 색을 지정하는데, 그동안 true_color 없으면
      무조건 검정 처리해 레이어별 색 구분(예: 청록 DATA 레이어)이 전부 뭉개졌다. `_resolve_aci()`
      +`ezdxf.colors.aci2rgb`로 실제 ACI 색 복원. ACI 7(흰색)은 처음엔 인쇄 관례로 무조건 검정
      처리했으나, 사용자 확인 후 **"화면=흰색, 인쇄=검정"**으로 재수정 — `_color()`는 흰색을
      그대로 반환하고, `pdf_export.py`에 `_swap_white_to_black_for_print()`/`_restore_swapped_colors()`
      신설해 렌더 직전 흰 계열 잉크색만 검정으로 바꿨다 렌더 후 원복(AutoCAD의 "화면 흰·종이
      검정" ACI7 관례를 그대로 재현). **ⓕ 휠줌 시 선택박스·핸들 화면크기 고정** — `_scale_or_1()`
      이 아이템 자체 `.scale()`만 보고 뷰의 줌 배율은 무시해, 확대할수록 선택 테두리·핸들이
      도형과 함께 커져 화면상 부담스러웠다(위 ⓐ 재스케일과 함께 실사용에서 발견). `_view_zoom_factor()`
      신설(씬에 붙은 뷰 중 `isInteractive()`인 메인 캔버스의 줌 배율만 반영 — `setInteractive(False)`인
      미니맵은 제외) + `_scale_or_1()` 2곳에 곱산. Figma·AutoCAD 관례(줌과 무관하게 화면 픽셀
...
**실조건 확인(2026-07-31)**: 사용자가 `python run.py`로 열 폭 드래그 자체는 정상 동작 확인.
같은 확인 중 무관한 기존 버그 발견·수정 — **다크모드에서 표 본문 글자가 거의 안 보임**:
헤더 행만 `_HEADER_FILL`(연회색) 배경이 있고 본문 행은 배경 없이(`NoBrush`) 어두운 캔버스
위에 고정 검정 잉크(`_INK`, 표는 색 편집 미지원 아이템이라 항상 고정)로 그려져 대비가
거의 없었음(`_TitleBlockItem`은 항상 흰 종이 배경이라 애초에 이 문제가 없었던 것과 대조).
`_BODY_FILL`(흰색) 도입해 표 전체 배경을 항상 흰색으로 고정(표제란과 동일 관례). 표 스모크
13종(신규 없음, 기존 회귀만) 통과 + 자체렌더로 배경 대비 확인. 편집 중 글자가 잘려 보이는
것은 `_CellEditor`(일반 `QLineEdit`)의 정상 커서 스크롤 동작 — 커밋 후 `paint()`의 자동
폰트축소+줄바꿈으로 해소되는 것으로 확인(버그 아님).
...
4. (계획서 §5 권장 흡수) — **전부 완료**(직교 커넥터+자동라우팅(A*·경유지 힌트)·심볼 라이브러리·포트/접속점):
   - ~~심볼/스텐실 라이브러리~~ — **완료**(6종 dock 팔레트 + 외곽선 스냅 + 중앙 라벨). 후속
     둘 다 완료: 진짜 드래그앤드롭(M3 #17) · **심볼 종류 추가**(2026-07-29, 커밋 `11f1f22`,
     실조건검증 ✓) — 표준 4종(수동입력·수동작업·화면출력·지연) + 방송/CCTV 결선도 픽토그램
     4종(카메라·증폭기·랙·안테나), 총 14종. `_SYMBOL_KINDS` 하나에 경로 팩토리만 추가하면
     팔레트·`.ecad`·DXF export·도형교체 메뉴가 전부 자동 반영되는 기존 설계 그대로 활용
     (host.py·document.py 무변경). Phase 5 `sketch_build.py`(Qt 비의존 이미지→도면 빌더)의
     kind 목록도 동기화. 자체렌더 중 "화면출력"이 기존 "시작/끝"(스타디움)과 구분 안 되는
     걸 발견해 좌측 오목·우측 볼록 비대칭 곡선으로 재설계. 스모크 285종(신규 2종 포함) 통과.
   - ~~포트/접속점~~ — **완료**(`1b06976`, 변 중점 4포트 우선 스냅 + 연속 폴백). 후속(8포트)도
     **완료**(2026-07-29, 실조건검증 ✓) — 상세는 아래 "신규기능 — 8포트 확장" 항목 참조.
...
경유는 BFS 재배치가 원본 위치를 버려 배제 — 완성도 우선). 빌더 API: `box`/`ellipse`/`symbol`(14종,
2026-07-29 8종 추가 — 아래 신규기능 항목 참조)/
...
**신규기능 — 캔버스-퍼스트 레이아웃(도킹 → 플로팅 패널) 완료(2026-07-29, 커밋 `616bcfb`·
`d967835`·`67fa346`, 실조건검증 ✓)** — 사용자가 다른 캔버스앱(Figma/Excalidraw) 스크린샷과
비교해 지적: 도형/레이어/속성/미니맵 `QDockWidget`이 콘텐츠 크기와 무관하게 dock 영역 칼럼
전체를 예약해 빈 공간이 크고, 하단 `QStatusBar`도 창 폭 전체를 가로지르는 빈 줄로 남는다는
문제. 원인은 Qt 도킹 모델 자체(칼럼 전체 예약, 콘텐츠가 작아도 캔버스로 안 넘어감)라
deep-interview로 스코프 확정 후 전면 전환: 도형+레이어(같은 카드 안 토글, 좌상단)·속성
(우상단)·미니맵(속성 아래)을 `_FloatingPanel`(QFrame, 콘텐츠 크기로 `adjustSize`, 위치
고정+접기 버튼 — 자유 드래그 재배치는 스코프 밖, Figma류도 위치 고정이 관례)로, 하단
상태바는 줌 배지(우하단)+토스트(하단중앙, `self.statusBar()`를 `_ToastLabel` 프록시로
오버라이드 — 기존 `.showMessage()` 호출부 20여 곳 무수정)로 대체. `_reposition_panels()`가
뷰 리사이즈·줌·레이어 변경 시 모든 패널 위치를 재계산(기존 `_reposition_floating_toolbar`와
같은 좌표 관례 재사용).
⚠ **실사용 피드백으로 발견·수정한 버그 2건**(2026-07-29): ⓐ 도형+레이어를 `QTabWidget`으로
묶었더니 내부 `QStackedLayout.sizeHint()`가 **탭 전환과 무관하게 두 페이지 중 큰 쪽 크기로
고정**되는 Qt 기본 동작 때문에, 콘텐츠가 짧은 레이어 탭을 봐도 패널이 도형 탭 크기 그대로
남아 빈 공간이 생겼다(실측: 두 탭 모두 274×348로 동일, 레이어 페이지 자체 sizeHint는
268×95에 불과). `QTabWidget`을 걷어내고 두 콘텐츠를 `setVisible()` 토글로 바꿔(숨긴 위젯은
일반 레이아웃 sizeHint 계산에서 제외됨 — 패널 접기 버튼과 같은 원리) 탭마다 정확히 콘텐츠
크기로 줄게 했다. 숨김→재표시 직후 `adjustSize()`를 부르면 레이아웃 무효화가 아직 반영 전이라
stale한 크기에 멈추는 2차 함정도 있어 `layout().activate()`를 먼저 호출해 해결. ⓑ 미니맵
인디케이터를 실제 가시 영역 비율대로 그렸더니 메인 뷰를 확대할수록 박스가 작아져 게임
미니맵(스타크래프트 등)과 다르게 동작한다는 지적 — 종횡비는 유지하되 크기는 줌과 무관하게
고정(`_INDICATOR_PX`=폭 30px)으로 변경. 미니맵 패널 자체 크기도 self-review 단계에서
190×150(가로긴형)→170×220(세로긴형)으로 수정 — `_MinimapView` 클래스 docstring이 이미
경고하던 "뷰포트가 가로로 넓으면 `fitInView(KeepAspectRatio)`가 레터박스돼 인디케이터가
작아 보이는" 함정을 반복할 뻔했다(옛 dock이 "좁고 세로로 긴" 비율로 고쳤던 이유와 동일).
⚠ **테스트 함정**: `CanvasWindow()`를 `.show()` 없이 생성하면 Qt 레이아웃 무효화 타이밍이
달라져 탭 전환 크기 축소가 오프스크린에서 재현되지 않는다(실조건은 항상 창이 떠 있어 이
...
`_shape_ports`에 bbox 대각 꼭짓점 4개(NE·SE·SW·NW)를 추가해 변 중점 4개(N·E·S·W)와 합쳐
8개로 확장(`_nearest_border`로 실제 외곽선 투영, 마름모는 대각 4점이 4변 중점에 역투영되는
역전 구조). 우선순위·시각 표현은 변/꼭짓점 구분 없이 균일. 실조건 테스트 중 사용자가 연쇄로
버그 5건을 더 발견해 같은 세션에서 함께 해결 — 결과적으로 8포트 자체보다 **라우팅 방향·
안정성 버그 수정이 이 세션의 본체**가 됐다:
  - **선택 도구 8포트 hover→커넥터 드래그** 완료 — 미선택 도형에 마우스만 올려도(화살표 도구
    없이) 8포트가 예고되고, 포트에서 드래그하면 도형 복제 없이 커넥터만 생성(`_hover_port_at`
    +`_hp_create_arrow`, 기존 qc-dot 4방향점과 별개 시스템). 포트가 도형 테두리 위라 "클릭=선택"
    과 자리가 겹치는 문제는 press를 보류했다가 **release 시점에 드래그 여부로 결정**(드래그
    했으면 커넥터, 안 했으면 평소처럼 선택)해 해결.
  - **마름모 N/E/S/W 법선 축정렬** — 좌우 꼭짓점 진입이 위아래로 드나들던 버그. 원인:
    `_nearest_border`가 그 지점의 '실제로 닿은 변 하나'에 수직인 법선을 주는데, 마름모 꼭짓점은
    두 변이 만나는 자리라 어느 변이 최근접이냐에 따라 법선이 기울어짐(폭≠높이일수록 심함,
    실측: 185×106 마름모 E포트가 (0.50,-0.87)로 나와 라우터 축판정이 "수직"으로 오판). N/E/S/W
    4개는 위치는 그대로 두고 법선만 의도된 축(N/S=수직·E/W=수평)으로 강제(`_CARDINAL_LOCAL_DIRS`
    +`_map_local_dir_to_scene`→이후 `_axis_forced_local_normal`로 통합).
  - **사각형 대각 꼭짓점 법선** — 위와 같은 부류(진짜 꼭짓점이라 변 선택이 임의) 버그가 사각형
    NE/SE/SW/NW에도 있었음(탐색 순서상 항상 세로 변이 이겨, 정사각형으로 테스트해도 4개 모두
    '수평'으로 나옴 — 도형 비율과 무관한 코드 우연). deep-interview 확정: **가까운 변 기준**
    (가로가 세로보다 길면 수평, 세로가 더 길면 수직 — 동률이면 수평)으로 강제.
  - **법선 보정 위치를 `_nearest_border` 자체로 이동** — 처음엔 보정을 `_shape_ports`에만
    넣었다가, 화살표를 새로 그리거나 스냅할 땐 맞는데 **가장 흔한 경로인 자동 라우팅**
    (`build_elbow`/`reroute`, 도형 이동 시마다 재계산)은 별도의 미보정 함수 `_bound_normal_scene`
    이 `_nearest_border`를 직접 불러 여전히 옛 버그를 그대로 갖고 있는 걸 실측으로 발견 —
    보정을 두 경로가 공유하는 `_nearest_border` 자체로 옮겨 포트 목록·자동 라우팅·도형 이동 후
    재계산 전부 일관되게 고침.
  - **끝점 드래그 재설계 — "새로 그리기"와 동일 취급** — 사용자 지적("도형에 붙은 직각 화살표를
    뗐다가 다른 경로에 붙이면 직각도 풀어지고 경로도 이상해진다")으로, 끝점 드래그 시 옛 목적지
    기준 중간 꺾임점을 보존하며 스텁만 patch하던 방식(이 세션 초반에 세웠던 결정)을 **뒤집었다**
    — 이유: 그 결정의 취지("무관한 변경에 손 안 댄 경로가 안 바뀌어야 한다")가 사용자가 **직접**
    끝점을 옮기는 이 경우엔 적용되지 않는다고 재판단. 이제 라이브 드래그·확정 모두 두 끝점만
    가지고 `_apply_routing()`에 전부 위임(새 화살표 그리기의 `set_ortho_preview`와 동일 패턴) —
    옛 목적지 기준 중간점을 버리므로 다른 도형 재부착도 항상 깨끗한 엘보. `_on_endpoint_drag_start`
    도 더는 auto_route를 끄지 않음(새로 그린 화살표처럼 이후 도형 이동에도 계속 자동 재라우팅).
    ⚠ **스턱루프 경유**(규칙 11-b, 총 3차 시도) — 1차: 매 프레임 스텁만 국소 재정렬(축 판정을
    옛 끝점↔이웃 상대위치로) → 2차: 축 판정을 새 스냅의 실제 법선으로 교체 → 그래도 실제
    다중프레임 드래그(도형 반대편을 스치기만 해도)에서 이웃점이 표류(550→20까지 실측) — 매
    프레임 판단이 서로의 결과를 덮어쓰는 구조 자체가 원인이라 **메커니즘을 전환**(3차: 위 "새로
    그리기 전체 재계산"으로 교체, 드래그 종료 시점 1회 재정렬 방안도 검토했으나 사용자가 "새로
    그리기와 동일하게"를 최종 선택).
  - **끝점 드래그 크래시 수정(실사용자 보고 — 화살표 머리 재부착 중 프로그램 종료)** — 근본원인:
    아이템의 `mousePressEvent`가 press 시점에 "몇 번째 점을 잡았는지"(`_drag_endpoint`)를
    **한 번만** 캡처해 드래그 내내 그대로 쓰는데, 위 재설계로 매 프레임 `_apply_routing()`이
    경로 전체를 재계산하면서 점 개수 자체가 프레임마다 바뀔 수 있게 됐다 — press 시점 값(예:
    4점 경로의 3번)이 다음 프레임에 점이 2개로 줄면 범위 밖 인덱스가 돼 그 프레임에서
    IndexError. 드래그가 "중간에 뚝 끊기는" 것처럼 보인 순간이 이 크래시였고, `_drag_endpoint`가
    정리되지 못한 채 남아 다음 클릭도 재차 크래시시켰다(콘솔 없이 더블클릭 실행이라 트레이스백
    없이 그냥 꺼짐). 해법: `_resolve_drag_endpoint()`가 "0이었나"만 기억해 매 프레임 실제
    유효한 인덱스(0 또는 `len(self._endpoints())-1`)로 재계산 — `_ArrowItem`(`_p1`/`_p2`)과
    `_PolyArrowItem`(`_pts`) 둘 다 옳게 동작하도록 폴리모픽 `_endpoints()`를 사용.
  - Rejected: 끝점 드래그 시 스텁만 국소 재정렬(다른 구간 보존) | 사용자 재검토로 기각 —
    "무관한 변경 보호"라는 원 취지가 사용자 자신의 직접 조작엔 적용 안 됨.
  - Rejected: 코너 포트 법선을 항상 특정 축으로 고정 | 사각형은 가까운 변 기준이 자연스러운
    반면 마름모 등 심볼은 대각 포트가 이미 변 중점에 투영돼 이 모호함이 없어 사각형만 대상.
  - Confidence: high(모든 근본원인을 실측 재현 스크립트로 확인 후 수정, 매 라운드 스모크
    전량 재실행). Not-tested: 매우 밀집된 실도면에서 매 프레임 전체 재계산의 체감 성능
    (`python run.py` 실사용으로는 문제 없음 확인, 정량 프로파일링은 안 함).

**커넥터 변타기 라우팅 수정 + 포트/핸들 UX 간소화 완료(2026-07-30, 프록시검증 — 스모크 304종 +
자체스크린샷, 실조건 대기, 커밋 `e2d3d42`)** — 실사용 중 발견·지적한 4건, 세 차례 AskUserQuestion으로
범위 확정 후 진행.
  - **변 타기 근본 수정** — 위 8포트 확장 세션에서 코너 포트의 '법선 방향'은 고쳤지만 '법선
    방향으로 얼마나 뻗어나가는지'(스텁 길이)는 base 경로(`_route_ortho`의 preferred,
    `_ortho_elbow`)에 전혀 없었다. 두 부착점 좌표가 우연히 비슷하면(코너뿐 아니라 연속폴백
    임의점도) 첫 구간 길이가 0에 가까워져 사실상 곧장 도형 변을 타는 것처럼 보였다. A* 우회
    (`_candidates`)가 이미 쓰던 `_normal_stub`(own-rect 팽창분까지 escape)을 base 계산 자체로
    옮겨 '항상 자기 도형 밖으로 스텁 → 그 다음 엘보'로 통일. 기존 무회귀 테스트가 "conn_rects
    유무와 무관하게 raw `_ortho_elbow`와 동일"을 검증하고 있어 그 자체가 스텁 부재를 보장하던
    회귀 테스트였음 — 스텁 추가에 맞춰 불변식을 '스텁+엘보와 동일'로 재정의
    (`test_route_ortho_clean_path_stub_then_elbow`·`test_route_ortho_pure` 등 갱신).
  - **화살표 기본 종류 곡선→직각** — 순서도 위주 사용에 맞춰 `current_arrow_kind` 초기값 변경.
    선택 후 종류 전환(플로팅 툴바 `⌐▾`)은 불변 — 상단바에 종류 선택지를 늘리지 말라는 UI 심플
    원칙(memory `ui-simplicity-principle`)과 충돌하는 "그리기 전 호버 플라이아웃" 안은 검토 후
    기각(AskUserQuestion, 기본값 변경만 채택).
  - **discrete 포트 8→4** — `_shape_ports`가 도로 변 중점 4개만 반환(대각 4개 제거). 대각 부착
    능력 자체는 `_border_snap_at`의 연속 폴백(`_nearest_border` 직접 호출, 목록과 무관)이
    그대로 지원해 무회귀 — 줄어드는 건 호버·선택 시 '상시 표시 discrete 점 개수'뿐.
  - **변 리사이즈 핸들 + qc-dot 통합** — 선택 시 점이 13개(꼭짓점4·변4·회전1·qc-dot4)나 되던
    것을 Lucid 스크린샷 대조로 지적받아, 변 중점의 온보더 리사이즈 사각과 그 바깥 qc-dot을
    qc-dot 위치의 겸용 점 하나로 합쳤다(8개로 축소). 뷰가 press를 가로채(`_qc_dot_at`) 첫 임계
    (8px) 초과 이동에서 그 변의 축 성분과 수직 성분을 비교해 축 우세=리사이즈(`_apply_box_resize`
    그대로 재사용, `item._box_resize`를 뷰가 직접 세팅), 수직 우세=기존 qc 드래그(커넥터만
    생성)로 분기. 클릭(이동 없음)은 기존 그대로 복제+화살표. AskUserQuestion으로 "완전 통합
    (고위험)"을 사용자가 명시 선택 — 아이템 자체 press/move(Qt 그랩)와 뷰 레벨 qc-dot 가로채기가
    서로 다른 메커니즘이라 병합엔 뷰가 아이템 메서드를 직접 호출하는 방식이 필요했음.
  - 호버 커넥터 포인트(선택도구 미선택 도형 포트) 커서를 이동(SizeAll)에서 십자선으로 — 예전엔
    '주석 위=이동' 분기로 떨어져 드래그가 이동이 아니라 커넥터 생성임을 커서로 못 알려줬다.
  - Rejected: 변핸들+qc-dot을 시각적으로만 분리(위치는 겹치되 동작은 기존 유지) | 사용자가
    완전 통합을 리스크 인지 후 명시 선택.
  - Confidence: medium(스모크 304종 + 자체 스크린샷 — 선택 시 점 배치까지 확인). Not-tested:
    실제 마우스 드래그 감각(리사이즈/커넥터 분기 임계 8px 체감, 변타기 수정의 시각적 개선) —
    `python run.py` 실조건 확인은 사용자가 이후 실사용 중 계속 확인하기로 함.

**선택 핸들 고정크기+hover 강조 · 정렬 가이드선 토글 완료(2026-07-30, 프록시검증 — 스모크
304종, 실조건 대기)** — 사용자가 Lucid 스크린샷과 대조해 선택 핸들(모서리·회전·qc-dot)이
작다고 지적, 1차로 획 두께 비례+[MIN,MAX] 클램프 하한을 5→8로만 올렸으나 재확인 결과 "선택된
도형의 핸들을 키우라는 뜻이 아니라, **선택도구로 미선택 도형에 호버할 때 뜨는 포트 예고점을
다시 hover하면 커지는 스냅 마커**(`_draw_snap_marker`, 지름 10)가 적당해 보인다"는 뜻이었음이
드러나 방향을 바꿨다. 최종: `_HandleResizeMixin`의 획 두께 비례 계산(`_HANDLE_FRAC`/
`_HANDLE_STROKE_FRAC`/`_stroke_width`)을 전부 걷어내고 고정값(`_HANDLE_PX`) 하나로 통일—
1차로 16(옛 상한)을 썼다가 "너무 커졌다"는 재피드백으로 10(포트 스냅 마커와 동일 지름)으로
축소. 크기를 고정하며 잃는 "어느 점이 잡히는지"는 hover 강조로 보완: 커서가 특정 핸들
위에 있으면 그 점만 반전(흰 채움+색 테두리 굵게, Figma류 hover 관례) — `_hover_handle_at()`이
핸들 종류별 키(모서리 인덱스·회전·qc-dot 변·화살표 끝점 인덱스)를 판정하면 뷰가 매 마우스
이동마다 계산해 해당 아이템의 `_hover_handle`에 저장하고 `paint()`가 그 키만 반전 렌더.
박스 핸들(네모·원)과 선/화살표 끝점 핸들 둘 다 동일 메커니즘 공유.
같은 세션에서 **정렬 가이드선(스마트 정렬 스냅+보라 참고선) 토글**도 추가 — 보기 메뉴 +
`Shift+A`(격자 토글 `Shift+G`와 동일 관례), `host.align_guides_enabled` 플래그로
`_apply_smart_snap()` 진입 자체를 막아 스냅 동작과 가이드선 표시를 한 몸으로 켜고 끈다(둘을
따로 뗄 이유가 없다고 판단). Rejected: 핸들 크기를 획 두께 비례로 유지하며 하한만 올림(1차
시도) | 기본 펜 두께가 이미 1px로 낮춰져(2026-07-28) 비례 계산이 항상 하한에 클램프되는
...
(`python run.py` 몫) — 아래 항목에서 실조건 확인 후 개선.

**색 선택 팝업 오피스류 그리드 단순화 완료(2026-07-31, 커밋 `ad4f01c`·`7923963`·`d4086b2`,
실조건검증 ✓)** — 위 도형 채우기 항목의 "✕" 버튼·`QColorDialog` 직결 UI가 복잡하다는 사용자
피드백(Excel/PowerPoint 색 선택창 스크린샷 3장 제시)으로 후속 진행. 채움·선색 스와치·플로팅
툴바 "⋯"가 공유하는 새 컴포넌트 `_ColorGridPopup`(`Qt.WindowType.Popup`, 바깥 클릭 시 자동
닫힘 — 2026-07-28 코드정리에서 삭제된 pasteflow 유산 `_ColorPalettePopup`과 동일 패턴)을
도입: 무채색 1열+유채색 6열(`QColor.lighter()/darker()`로 밝기 3단 생성, 각 열 위→아래
표준→연한→어두움 순) + 최근 사용한 색 1열(최대 3개, QSettings로 영구 기억) + 클릭 즉시 적용.
채움의 "없음"은 별도 외부 버튼 대신 팝업 안 "Ø" 항목으로 흡수(`_pf_fill_none` 제거,
`_clear_fill`은 그 항목이 호출하는 메서드로 유지).

"다른 색…"은 새 피커를 만들지 않고 기존 `QColorDialog` 그대로 재사용하되(사용자가 스크린샷으로
"오른쪽 그라디언트+RGB/16진수는 이미 있으니 새로 만들 필요 없다"고 직접 확인 — 규칙 2 손안의
카드), 왼쪽 열(Basic colors 48색 그리드·Pick Screen Color·Custom colors 16칸·Add to Custom
Colors — 우리 그리드와 중복)만 숨긴다(`_strip_color_dialog_left_column`). 알파 채널이 필요한
채움은 Qt가 자동으로 비-네이티브 다이얼로그로 전환해(네이티브가 알파 미지원) 위젯 트리 조작이
가능하지만, 알파가 없는 선 색은 원래도 네이티브라 이 처리 자체가 스킵된다.
⚠ **v1(고정 좌표 x<265·폭<=260) → v2(라벨 앵커) 교체**: v1은 격리된 헤드리스 테스트에서만
실측한 픽셀 값이라, 실제 앱(전역 `app.setStyle("Fusion")` 컨텍스트)에서는 내부 레이아웃 폭
자체가 달라 오른쪽 그라디언트 사각형·색상 슬라이더까지 함께 숨겨버렸다(사용자가 `python run.py`
스크린샷으로 발견). v2는 절대 좌표 대신 "&Basic colors"/"&Custom colors" 라벨과 두 버튼(Qt
내부 고정 영문 문자열 — 이 앱은 Qt 자체 번역 미탑재라 로캘 무관)을 앵커로 찾고, 그 앵커들과
같은 x·같은 세로 범위에 있는 이름 없는 위젯(실제 색상 그리드)만 같이 숨기는 방식 — 오른쪽
열은 구조적으로 다른 x라 폰트·DPI가 달라져도 안전. 앵커가 하나라도 안 잡히면 아무것도 안
숨기고 옛 모양 그대로(안전 폴백). 회귀 테스트 작성 중 자체 버그도 발견: `QLabel`이 Qt
상속구조상 `QFrame`의 서브클래스라 `isinstance(w, QFrame)`으로 그라디언트를 찾으면 라벨을
잘못 집음 — `type(w) is QFrame`으로 교체.
스모크 323→328종. Rejected: 알파 포기하고 네이티브 다이얼로그로 통일(사용자가 "있는 걸 쓰고
싶다"며 비-네이티브 다듬는 쪽 명시 선택, 반투명 채움 유지) · 자체 그라디언트 피커 개발(이미
Qt 안에 있음 확인). **실조건검증 ✓**(2026-07-31, 사용자가 `python run.py`로 그리드 팝업·
최근색 열·간소화된 "다른 색" 창의 그라디언트 사각형 유지 전부 확인 — "해결됨").
