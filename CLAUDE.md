...
**신규기능 — 그리드/스냅투그리드 완료(2026-07-28, 커밋 `16c7551`, 프록시검증 — 실조건 대기)** —
...
트레일러 참조. **실조건(`python run.py`) 미확인** — GUI 상호작용 느낌(드래그 시 스냅 감각)은 사용자
확인 필요.
...
**신규기능 — 미니맵 완료(2026-07-28, 커밋 `bd9637d`, 프록시검증 — 실조건 대기)** — 무한캔버스
...
⚠ **함정(실조건 스크린샷으로 발견·수정, 커밋 `2bd6827`):** 사용자가 `python run.py`로 확인 중
인디케이터가 실제 보이는 영역과 다른 자리(구석)에 그려지는 것을 스크린샷으로 제보. 원인 —
`_refresh_minimap()` 훅이 `CanvasWindow.resizeEvent`(창 자체 리사이즈)·줌·스크롤바뿐이라, **dock
스플리터를 드래그해 메인 뷰포트 크기만 바뀌는 경우**(창 크기는 그대로) 어느 훅도 안 걸렸다.
`drawForeground`는 페인트마다 라이브로 다시 계산해 값 자체는 항상 맞지만, 그 페인트가 다시
스케줄 안 되면 화면엔 옛 인디케이터가 남는다 — **계산이 옳아도 "언제 다시 그릴지" 트리거가
누락되면 결과가 틀려 보인다.** 해법: 이미 감시 중이던 `self._view.viewport()`의 `eventFilter`에
`QEvent.Type.Resize` 분기 추가 — 원인(창 리사이즈든 dock 리사이즈든) 불문 뷰포트 자체 크기 변화를
항상 잡는 구조적 신호라 개별 트리거를 더 나열할 필요가 없다. Confidence: medium(사용자의 정확한
재현 조작은 미확인, 구조적 빈틈이었다는 정황 근거).
