...
- **Lucid 대조 캔버스 UX 개선 시리즈(§8 항목16, 2026-08-03)** — 호버 예고점 잔상/불규칙,
  미선택 도형 내부 클릭·이동커서, 테두리 변 전체 리사이즈, 핸들 테두리 밖 오프셋, 선택 표시
  2갈래(단일·화살표=얇은 중심선 / 다중=바깥 밴드), 직각 커넥터 세그먼트 알약 분할-드래그,
  화살촉 어깨 모따기 수정까지 완료. **미완료**: 미선택 도형 테두리 연속 호버(§8 항목16).
- **다음 순서**: 위 §8 항목13~16(안테나·PDF 메뉴·AI 이미지·테두리 연속 호버) + 기존 "다음
  순서"·"큰 설계 필요" 목록(메뉴 전용 아이콘 8종 SVG화·실도면 대규모 성능 등) —
