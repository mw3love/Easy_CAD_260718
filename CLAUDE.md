...
- **밀집 도면(1600개) 성능 최적화 3~4단계**(§8 항목0 후속, 2026-08-08~09) — "버벅거림 최소화"
  ...
  **4단계**(2026-08-09): 유일하게 남은 60fps 초과 시나리오(뷰 렌더·전체축소, 5.2배)를
  ...
  남은 격차(~80ms, 목표 16.7ms)는 안티에일리어싱/LOD처럼 **시각 결과 자체를 바꾸는** 축이
  유일한 다음 후보라 사용자 확인 후 진행 예정(픽셀 완전일치 게이트 재기준선 필요). 상세:
  `docs/history/2026-08.md` "밀집 도면 성능 4단계" 참조, 함정 `docs/pitfalls.md`
  "렌더링(QPainter/QIcon/오프스크린)".
- **다음 순서**: §8 항목17(범용 CAD TRIM/EXTEND, 스코프 미정) + 위 성능 4단계 후속(AA/LOD
  시각 트레이드오프 결정) + 기존 "다음 순서"·"큰 설계 필요" 목록(메뉴 전용 아이콘 8종 SVG화 등)
