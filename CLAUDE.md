...
— 상세 로드맵 `docs/EasyCAD_계획.md` §Phase 6.
  ...
  - **M4-4 직교 커넥터 고도화 + #4 라우팅 드롭다운** 진행(실조건검증 ✓ 2026-07-22, 미커밋):
    ...
    **남은 것(다음 세션):** ⓐ **경로가 도형 변을 타는 경우 분리**(도형 관통 회피와 연관, Lucid도 실사용선
    관통 드물어 우선순위 낮음), ⓑ **반경 슬라이더 UI**(모델 `_curve_r` 준비됨), ⓒ **곡선 화살표(베지어)
    완전 통합**(데이터모델 이질성 `_ctrl` vs `_pts`, 큰 작업 — 오래 deferred), ⓓ **도형 내부 빈공간 이동**
    (Lucid/FigJam식, 선택 시 내부를 hit에 포함 — 커넥터와 무관한 별개 UX).
