...
- **다음 순서**: §8 항목14~15(PDF 메뉴·AI 이미지) + 항목17(범용 CAD TRIM/EXTEND, 스코프
  미정) + 기존 "다음 순서"·"큰 설계 필요" 목록(메뉴 전용 아이콘 8종 SVG화·실도면 대규모
  성능 등) — `docs/EasyCAD_계획.md` §8 참조.
- **웹 트랙 병행개발(2026-08-05 시작)** — `web_prototype/`(이 리포 하위, PyQt 본체와 별개 코드,
  `easycad/`는 안 건드림)는 Python(PyQt) 앱과 **둘 다 끝까지 만드는** 것이 목표다. 웹 쪽이 늦게
  시작해 진도가 뒤처져 있어 당분간 웹 개발에 자원을 집중하는 것이지, "웹 전환 여부를 저울질하는
  실험"이 아니다(2026-08-05 사용자 확인 — 과거 "결론 미정"류 표현 쓰지 말 것). 시작은 "playwright
  ...
  Python 유지, 캔버스/인터랙션만 웹). 로컬로 보려면 `web_prototype/`에서
  `python -m http.server 8791` 후 `localhost:8791/index.html`.
  경위·근거는 `docs/history/2026-08.md` "웹 전환 실험" 항목들(2026-08-05 두 건), pitfalls는
