...
- **웹 전환 실험 진행 중(2026-08-05 시작 → 병행개발 결정, 세부 결론 미정)** — `web_prototype/`
  ...
  자동검증 통과, 17도형·22화살표 규모에서 A* 폴백 대각선 버그 1건 발견+수정, **성능 미해결
  사항 발견**(매 pointermove 전체 화살표 재계산이 이 규모에서 이미 60fps 예산 초과 — 무효화
  최적화 필요, 다음 라운드 과제). 추천 방향은 전면 재작성이 아니라 하이브리드(DXF/PDF는 기존
