...
— 상세 로드맵 `docs/EasyCAD_계획.md` §Phase 6.
  ...
  - **M4-4 잔여 ⓓ·ⓑ 완료**(실조건검증 ✓ 2026-07-24):
    ...
    **남은 것:** ⓐ **경로가 도형 변을 타는 경우 분리**(도형 관통 회피와 연관, Lucid도 실사용선
    관통 드물어 우선순위 낮음), ⓒ **곡선 화살표(베지어) 완전 통합**(데이터모델 이질성 `_ctrl` vs `_pts`,
    큰 작업 — 오래 deferred).
    **후속 검토(사용자 제안 2026-07-24):** 「직각 엘보」와 「곡선 반경 0」이 중복 → *메뉴*가 아니라
    *내부 모드*를 통합하는 쪽 권장(`_routing`을 straight/ortho 둘로 줄이고 직각=반경0 프리셋). 지금
    직각은 선분 클리핑, 곡선은 QPainter 클립으로 **같은 그림을 두 코드로** 그린다(라벨 갭). 메뉴에서
    직각만 빼는 안은 사용자 손해(곡선 선택+▾ 5번)만 크고 코드 이득 0이라 기각.
