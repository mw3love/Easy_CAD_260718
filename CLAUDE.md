...
- **웹 전환 실험 진행 중(2026-08-05 시작 → 병행개발 결정, 세부 결론 미정)** — `web_prototype/`
  (이 리포 하위, PyQt 본체와 별개 코드, `easycad/`는 안 건드림)에 포트/커넥터+지속연결+다중선택
  상호작용을 SVG+DOM으로 재현해 "playwright 자동검증만으로 버그를 화면 없이 잡을 수 있는가"
  가설을 검증 중. 1차: 자동검증만으로 실버그 1건(A* 그리드반올림 대각선). 2차: 지속연결(도형
  이동→화살표 재라우팅) 1차 구현에서 버그 없이 통과. **사용자가 실제 브라우저로 손맛 확인
  완료("나쁘지 않다")** — Python·웹 병행 개발 결정, 당분간 웹 쪽에 개발 집중. 3차(인터랙션
