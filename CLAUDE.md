...
— 상세 로드맵 `docs/EasyCAD_계획.md` §Phase 6.
  ...
  - **M4-4 ⓐ 최종 잔여(변타기/관통) 완료(실조건검증 ✓ 2026-07-27, 커밋 `75c60c6`)** — 계획서가
    ...
    **이어서 실조건서 부착점 이탈·흔들림 2건 추가 발견 → Stage4(`_absorb_near_alignment`) 철회** —
    ...
    ⚠ **미해결로 남은 버그(다음 세션 우선 항목)**: 화살표를 뗐다가 도형 변의 **중심점이 아닌 곳**에
    재부착하면, 그 도형을 옮겨도 화살표가 안 따라온다(사용자 실조건 재현, 2026-07-27). 코드 리딩
    으로 F8(Ortho)·Shift 드래그 분기가 `set_bound`를 안 거치는 버그를 찾아 고쳤으나(`_rebind_
    at_fixed_point` 신설, 스모크 3종 추가) **사용자 재현에서는 여전히 재현됨** — 이 수정이 실제
    트리거를 못 맞혔거나 다른 분기가 더 있다는 뜻. 일반(비-F8) 재부착 경로는 도형 5종·양끝·7개
    비포트 위치로 스윕해도 재현 안 됨(오프스크린 시뮬레이션 한계일 가능성 — 실제 마우스 이벤트
    시퀀스와 미세하게 다를 수 있음). **다음 세션은 새 가설로 패치하기 전에 정확한 재현부터**
    (화면 녹화 또는 클릭·드래그·릴리스 좌표 순서) — 규칙 11-b 스턱루프 트립와이어 발동 지점.
